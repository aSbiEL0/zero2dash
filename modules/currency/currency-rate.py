...
from display_layout import LAYOUT_HALF, ROW_HEIGHT, aligned_text_x, centred_text_y
...
def render_currency_image(background_path: Path, output_path: Path, display_date: str, status: str, snapshot: RateSnapshot | None, message: str | None) -> None:
    ...
    if status == "ok" and snapshot is not None:
        ...
        rate_font = _fit_font(draw, rate_text, width_limit=content_column.width - 20, initial_size=96, bold=True)       # -20 #
        gap = 0                                     # Letter spacing
        rate_size = getattr(rate_font, "size", 70)
        suffix_text = "zł"

        while True:
            suffix_font = load_font(max(50, int(rate_size * 0.75)), bold=True)
            ...
            if total_w <= (content_column.width - 8) or rate_size <= 10:
                break
            rate_size -= 2
        ...
        _draw_text_with_shadow(
            draw,
            (aligned_text_x(content_column, pair_font, pair_text, "center"), pair_y),
            pair_text,
            font=pair_font,
            fill=white,
            shadow_fill=shadow,
        ...
        _draw_text_with_shadow(
            draw,
            (aligned_text_x(content_column, date_font, display_date, "center"), date_y),
            display_date,
            font=date_font,
            fill=white,
            shadow_fill=shadow,
        ...
        message_font = _fit_font(draw, message_text, width_limit=content_column.width, initial_size=max(28, width // 10), bold=True)
        _draw_text_with_shadow(
            draw,
            (aligned_text_x(content_column, pair_font, pair_text, "center"), pair_y),
            pair_text,
        ...
        _draw_text_with_shadow(
            draw,
            (aligned_text_x(content_column, message_font, message_text, "center"), centred_text_y(message_font, message_text, LAYOUT_HALF.row_top(1) + ROW_HEIGHT)),
            message_text,
            font=message_font,
            fill=white,
            shadow_fill=shadow,
